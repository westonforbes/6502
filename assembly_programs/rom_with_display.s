...
write_data_to_display:
    ...
    lda #%11000000      ; Set E (Enable) = 1 to enable, RS = 1 (data), RWB = 0 (write).
    sta PORTA           ; Write to VIA port A.
    lda #%01000000      ; Set E (Enable) = 0 to disable, RS = 1 (data), RWB = 0 (write).
    sta PORTA           ; Write to VIA port A.
...
write_message:
    ...
    lda #','            ; Load character ','.
    sta PORTB           ; Write to VIA port B.
    lda #' '            ; Load character ' ' (space).
    sta PORTB           ; Write to VIA port B.
    lda #'W'            ; Load character 'W'.
    sta PORTB           ; Write to VIA port B.
    lda #'O'            ; Load character 'O'.
    sta PORTB           ; Write to VIA port B.
    lda #'R'            ; Load character 'R'.
    sta PORTB           ; Write to VIA port B.
    lda #'L'            ; Load character 'L'.
    sta PORTB           ; Write to VIA port B.
    lda #'D'            ; Load character 'D'.
    sta PORTB           ; Write to VIA port B.
    lda #'!'            ; Load character '!'.
    sta PORTB           ; Write to VIA port B.

loop:
    ; Load alternating bit pattern 0x55 to port B.
    lda #$55       ; Load register A with 0x55.
    sta #$6000     ; Write value to VIA port B.
    nop            ; No operation (delay).
    nop            ; No operation (delay).
    nop            ; No operation (delay).

    ; Load alternating bit pattern 0xAA to port B.
    lda #$AA       ; Load register A with 0xAA.
    sta #$6000     ; Write value to VIA port B.
    ...
    .word start    ; Set reset vector to start of program at 0x8000.
